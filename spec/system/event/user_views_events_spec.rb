...
describe 'User views their buffet events' do
  it 'shouldnt be able to see edit event buttons on event page' do
    #Arrange
    ...
    expect(page).not_to have_content('Adicionar preços')
  end
  ...
  context 'as a buffet owner' do
    ...
    it 'can see more information about an event from the buffet events page' do
      ...
      expect(page).to have_content('10 pessoas Mínimo')
      expect(page).to have_content('20 pessoas Máximo')
      expect(page).to have_content('120 minutos')
    end
  end
end